...
<Tabs>
  ...
  <Tab title="Default serve (production API)">
    ...
    pnpm start
    ```
    Runs `ng serve --configuration production`. Angular replaces `environment.ts` with `environment.prod.ts`, so all API calls go to the live Railway backend at `https://galactic-tournament-app-production.up.railway.app`. Useful for running the SPA locally against the real production API without a local backend server.
  </Tab>
